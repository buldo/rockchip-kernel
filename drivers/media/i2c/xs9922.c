...
{
	{
		...
		.reg_list			= xs9922_AHD_720p_25fps,
		.mipi_freq_idx		= 0,
		.bpp				= 8,
		.hdr_mode			= NO_HDR,
		.lanes				= 4,
	...
	},
	{
		...
		.global_reg_list	= xs9922_init_cfg,
		.reg_list			= xs9922_AHD_720p_30fps,
		.mipi_freq_idx		= 0,
		.bpp				= 8,
		.hdr_mode			= NO_HDR,
		.lanes				= 4,
	...
	},
	{
		...
		.global_reg_list	= xs9922_init_cfg,
		.reg_list			= xs9922_AHD_1080p_25fps,
		.mipi_freq_idx		= 0,
		.bpp				= 8,
		.hdr_mode			= NO_HDR,
		.lanes				= 4,
	...
	},
	{
		...
		.global_reg_list	= xs9922_init_cfg,
		.reg_list			= xs9922_AHD_1080p_30fps,
		.mipi_freq_idx		= 0,
		.bpp				= 8,
		.hdr_mode			= NO_HDR,
		.lanes				= 4,
	...
	},
	{
		...
		.max_fps			= {
			.numerator			= 10000,
			.denominator		= 500000,			//需要奇偶行合并
		},
		.global_reg_list	= xs9922_init_cfg,
		.reg_list			= xs9922_cvbs_960x576_25f_P,
		.mipi_freq_idx		= 0,
		.bpp				= 8,
		.hdr_mode			= NO_HDR,
		.lanes				= 4,
	...
	},
	{
		...
		.max_fps			= {
			.numerator			= 10000,
			.denominator		= 600000,			//需要奇偶行合并
		},
		.global_reg_list	= xs9922_init_cfg,
		.reg_list			= xs9922_cvbs_960x480_30f_N,
		.mipi_freq_idx		= 0,
		.bpp				= 8,
		.hdr_mode			= NO_HDR,
		.lanes				= 4,
...
#define config_file 			"/userdata/board.conf"
char			board_config_buf[1024] =
{
	0
};
...
static int read_config(void * data)
{
	...
	if (IS_ERR(fp)) {
		...
		if (strlen(board_config_buf) > 0) {
			...
			char *			inf = strstr(board_config_buf, "interface=");
			...
			if (NULL != mode) {
				...
				for (i = 0; i < xs9922->cfg_num; i++) {
					int 			fps =
						 supported_modes[i].max_fps.denominator / supported_modes[i].max_fps.numerator;

					if (width == supported_modes[i].width && height == supported_modes[i].height && vrefresh == fps) {
			...
			if (NULL != timing) {
				...
				sscanf(timing, "outiming=%s %d %d %d %d %d %d %d %d %d %d %d",
					mode.name, &mode.vrefresh,
					&mode.hdisplay, &mode.hsync_start, &mode.hsync_end, &mode.htotal,
					&mode.vdisplay, &mode.vsync_start, &mode.vsync_end, &mode.vtotal,
					&mode.clock, &mode.flags);

				if (strlen(mode.name) > 0)
					hdmiCusMode = &mode;
			}

			if (NULL != inf) {
				struct regval * regs = (struct regval *) (xs9922->cur_mode->reg_list);

				if (strstr(inf, "tvi") != NULL || strstr(inf, "cvi") != NULL) {
					for (i = 0; regs[i].addr != REG_NULL; i++) {
						if (regs[i].addr == 0x010c) {
							regs[i].val 		= 0x00;
							break;
						}
					}
				}
				else if (strstr(inf, "ahd") != NULL) {
					for (i = 0; regs[i].addr != REG_NULL; i++) {
						if (regs[i].addr == 0x010c) {
							regs[i].val 		= 0x01;
							break;
...
static const struct xs9922_mode * xs9922_find_best_fit(struct xs9922 * xs9922,
	...
	struct v4l2_mbus_config * cfg)
{
	...
	cfg->flags			= V4L2_MBUS_CSI2_4_LANE | V4L2_MBUS_CSI2_CONTINUOUS_CLOCK | V4L2_MBUS_CSI2_CHANNELS;
...
static __maybe_unused int xs9922_auto_detect_hotplug(struct xs9922 * xs9922)
{
	...
	xs9922->detect_status =
		 ((~(val0 >> 4)) & 0x01) | ((~(val1 >> 4)) & 0x01) << 1 | ((~(val2 >> 4)) & 0x01) << 2 | ((~ (val3 >> 4)) & 0x01) << 3;
#endif
	...
	while (!kthread_should_stop()) {
		if (xs9922->power_on) {
			...
			if (xs9922->last_detect_status != xs9922->detect_status) {
				...
				for (ch = 0; ch < 4; ch++) { //遍历4个通道

					if (bits & (1 << ch)) { //判断变化的通道

						if (xs9922->detect_status & (1 << ch)) {
...
struct v4l2_ctrl_ops xs9922_ctrl_ops = {
...
struct v4l2_subdev_internal_ops xs9922_internal_ops = {
	.open = xs9922_open,
		};
...
struct v4l2_subdev_video_ops xs9922_video_ops = {
...
struct v4l2_subdev_pad_ops xs9922_subdev_pad_ops = {
...
struct v4l2_subdev_core_ops xs9922_core_ops = {
...
struct v4l2_subdev_ops xs9922_subdev_ops = {
	...
	const struct i2c_device_id * id)
{
	...
	if (!IS_ERR(xs9922->pinctrl)) {
		xs9922->pins_default = pinctrl_lookup_state(xs9922->pinctrl,
			OF_CAMERA_PINCTRL_STATE_DEFAULT);

		if (IS_ERR(xs9922->pins_default))
			dev_info(dev, "could not get default pinstate\n");

		xs9922->pins_sleep	= pinctrl_lookup_state(xs9922->pinctrl,
...
struct dev_pm_ops xs9922_pm_ops = {
...
struct of_device_id xs9922_of_match[] = {
	{
		.compatible 		= "xs9922"
	},
	{
...
struct i2c_device_id xs9922_match_id[] = {
	{
		"xs9922", 0
	},
	{
...
struct i2c_driver xs9922_i2c_driver = {
	.driver = {
		.name				= XS9922_NAME,
